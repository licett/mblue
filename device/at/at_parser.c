...
int at_prefix_in_table(const char *line, const char *const table[])
{
...
static enum at_response_type generic_line_scanner(const char *line, size_t len, struct at_parser *parser)
{
    ...
    if (at_prefix_in_table(line, urc_responses) >= 0)
        return AT_RESPONSE_URC;
    else if (at_prefix_in_table(line, final_ok_responses) >= 0)
        return AT_RESPONSE_FINAL_OK;
    else if (at_prefix_in_table(line, final_responses) >= 0)
...
static void parser_append(struct at_parser *parser, char ch)
{
    ...
    if (parser->state != STATE_RAWDATA && parser->cbs->check_data)
        parser->cbs->check_data(parser->priv);
}
...
static void parser_handle_line(struct at_parser *parser)
{
    ...
    if (type != AT_RESPONSE_FINAL_OK &&
            (type == AT_RESPONSE_URC || parser->state == STATE_IDLE))
    {
